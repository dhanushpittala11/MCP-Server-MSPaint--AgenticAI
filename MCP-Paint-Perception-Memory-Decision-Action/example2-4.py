...
async def draw_rectangle(x1: int, y1: int, x2: int, y2: int) -> dict:
    ...
    try:
        ...
        paint_window.click_input(coords=(640, 109 )) #640, 109
        time.sleep(1)
        ...
        time.sleep(1)
        canvas.press_mouse_input(coords=(x1, y1)) #x1+2560, y1
        time.sleep(1)
        canvas.move_mouse_input(coords=(x2, y2))  #x2+2560, y2
        time.sleep(1)
        canvas.release_mouse_input(coords=(x2, y2)) #x2+2560, y2
        time.sleep(1)
...
#         paint_window.click_input(coords=(428, 110)) #428, 110
#         time.sleep(0.5)
...
#         canvas.click_input(coords=(800, 400))
#         time.sleep(0.5)
        
#         # Type the text passed from client
#         paint_window.type_keys(str(text))
#         time.sleep(0.5)
        
#         # Click to exit text mode
#         canvas.click_input(coords=(1050, 650))
...
# @mcp.tool()
# async def add_text_in_paint(text: str) -> dict:
#     """Add text in Paint using keyboard shortcut for text tool"""
#     global paint_app
#     try:
#         if not paint_app:
#             return {
#                 "content": [
#                     TextContent(type="text", text="Paint is not open. Please call open_paint first.")
#                 ]
#             }
...
#         # 2. Use keyboard shortcut to activate text tool
#         paint_window.type_keys('t')  # Select Home tab
#         time.sleep(0.1)
#         paint_window.type_keys('x')  # Select Text tool
#         time.sleep(0.5)
...
#         # 4. Type the text
#         paint_window.type_keys(str(text), with_spaces=True, pause=0.05)
#         time.sleep(0.5)

#         # Optional: Click somewhere to deselect text box
#         canvas.click_input(coords=(1050, 650))

#         return {
#             "content": [
#                 TextContent(type="text", text=f"Text '{text}' typed successfully into Paint")
#             ]
#         }
...
async def add_text_in_paint(text: str) -> dict:
    ...
    try:
        ...
        # ✅ Step 1: Click on the Text Tool directly (DON’T use keyboard shortcuts!)
        paint_window.click_input(coords=(428, 110))  # Adjust as per your Paint version
        time.sleep(1)

        # ✅ Step 2: Click inside the drawn rectangle
        canvas = paint_window.child_window(class_name='MSPaintView')
        ...
        canvas.click_input(coords=(800, 400))  # Where your rectangle is
        time.sleep(1)

        # ✅ Step 3: Type the text safely
        safe_text = str(text).replace('%', '%%')
        paint_window.type_keys(safe_text, with_spaces=True, pause=0.05, set_foreground=True)
        time.sleep(1)

        return {
            "content": [TextContent(type="text", text=f"Text '{safe_text}' typed successfully into Paint")]
        }
